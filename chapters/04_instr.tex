...
(e.g. is the pointer really invalid?). The method worked fine if only one
plugin was available, because the condition was evaluated according to one
reply from this plugin, but when we added more plugins, we ran into problems.
We needed the conditions that ask if some statement may hold to evaluate to
\emph{true} if all plugins answeres \emph{true} and at the same time we needed
the conditions that ask if some statement really holds to evaluate to
\emph{true} if at least one plugins anseweres \emph{true} because of the
over-approximations.
...
can reply with a string. A condition is satisfied if and only if the result of
the query equals one of the values in the list of expected results.

...
\begin{description}
    ...
    \texttt{return} instruction of the given function, \texttt{in} cannot be
    set to "*" in this case).
    ...
    where the first element is the name of a condition and other elements are
    ...
    \item[\texttt{setFlags}] List of pairs \texttt{<flag, value>} that sets
      all \texttt{flags} to the corresponding \texttt{value} if the rule was
...
\noindent A \texttt{globalVariablesRule} is a JSON object with the following fields:
\begin{itemize}
...
Variable \texttt{<t>} will contain the value of the operand of \texttt{load}
instruction in the scope of the current rule and it can be used e.g. as an

...
Currently, following queries are available \todo{apply refactoring}:
\begin{description}
  ...
  \item[pointsTo(p,a)] Does a pointer \texttt{p} point to a value \texttt{a}?
  \item[isNull(p)] Does the given pointer \texttt{p} point to a \emph{null}
  value?
\end{description}
...
\begin{itemize}
  ...
  \item the beginning and end of each phase

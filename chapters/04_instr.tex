...
functions translated to LLVM whose calls will be inserted into the code and a
...
any other demanded instruction can be wrapped in the called function.  The tool
first loads a~whole module that is to be instrumented and begins to process it.
The instrumentation can be divided into phases; in each phase it goes through
all instructions and it looks if they match any instrumentation rule. If a
match is found, the rule is applied (i.e.~a~new code is inserted according to
the rule). The result of the instrumentation process is again an LLVM program.
...
the same interface. The interface describes a set of callbacks that
correspond to the queries and return string values. For example, a
query "Do pointers \texttt{p1} and \texttt{p2} point to the same memory
location?" can correspond to a callback \texttt{pointersAreEqual(p1,
...
handler for this query in class \texttt{Analyzer}.
...
\begin{tabular}[h]{>{\bfseries}p{4.2cm} | p{7.8cm}}
  \texttt{file}                & Path to a a file with definitions of instrumentation functions. \\
  \hline
  \texttt{analyses}            & List of paths to analyses plugins. \\
  \hline
...
\noindent A \texttt{globalVariablesRule} is an JSON object with following fields:
\begin{itemize}
...
of instructionaccording to \texttt{where} field. For now, we support only
...
\end{lstlisting}
\caption{Instrumentation of a code where bitcast is needed. The
...
as a parameter, but the value that we want to pass as this parameter
(\texttt{result}) is a 32-bit integer. Therefore, instruction \texttt{bitcast
...
given value is stored to an auxiliary list and can be used in later phase of
instrumentation.
